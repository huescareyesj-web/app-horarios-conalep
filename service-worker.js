...
// (cuando la app sigue abierta en primer plano).
...
// Aviso real del servidor (Web Push): esto funciona aunque la app esté cerrada,
// mientras el navegador siga corriendo en segundo plano (o la app esté instalada).
self.addEventListener('push', (event) => {
  let payload = { title: 'Empieza tu clase', body: '' };
  try {
  ...
  event.waitUntil(
    self.registration.showNotification(payload.title || 'Mi Horario', {
      body: payload.body || '',
      icon: 'icons/icon-192.png',
      badge: 'icons/icon-192.png',
      tag: 'clase-actual',
